...
async function authenticatedHandler (req, res, next) {
  ...
  try {
    await Promise.all([
      download(req.body.study.aggKey, __dirname + '/aggpublic.pem'),
      download(req.body.study.key, __dirname + '/respublic.key')
    ]);
  } catch (err) {
    console.log(err)
  }
  ...
  */
  ...
  const [_, fnName, fnVar, fnOperator, fnLiteral] = match;
